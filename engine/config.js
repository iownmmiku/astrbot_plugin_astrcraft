...
const DEFAULTS = {
  ...
  /** 单个技能最长执行时间（毫秒）
   *  300000 → 480000（对齐现实；**「她必须爬回地面」的断言一个字没改**）
   *
   *  实测账本：爬升六修（假到达手动补位/停滞误杀/落地读数/超时砍半/跳过徒劳策略）之后，
   *  连续 5 次失败**全部是时钟**：h15「技能执行超时」共爬 6 格、j20 测试先到点、
   *  k25 测试取消、k29「技能执行超时」**差 4 格到顶** ——
   *  极限竖井几何 ~15-20 秒/格 × 11 格 + 挖矿 ~100 秒 ≈ 需要 5 分钟多。
   *  给到 480 秒 = 等待预算对齐现实；挂死兜底仍在（watchdog 5→8 分钟）。 */
  skillTimeoutMs: 480000,
  /**
