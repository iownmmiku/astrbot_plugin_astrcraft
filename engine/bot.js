...
class McEngine {
  ...
  _startLoops() {
    ...
    // 反射层：**200ms 一次**（身体层设计 S1，见 docs/BODY_LAYER.md）。
    //
    // 本能的最坏延迟原来是**整整 1 秒**——被怪贴脸、溺水、着火都可能等满 1 秒，
    // 而"错过一秒就必死"的坠落早就证明 1 秒太慢（所以 MLG 单独拆到 100ms），
    // 其它本能没有理由享受更差的待遇。
    //
    // **这条改动的经过（两次才成）**：
    //   第一次提速后 `test_unstuck` 的"完全被埋住能自救"连续 3 次失败，
    //   于是回退。查下来是**两个独立原因**：
    //     ① 脱困的"二次确认"用**方块的确切坐标**当 key，而她被埋住时会被
    //        服务器推动、坐标一直微动 → 确认永远凑不齐两次。
    //        换成"被几格围住 + 都是什么方块"（不含坐标）之后好了。
    //     ② `test_unstuck` **自己**不稳定（60~80%）：它不验证场景是否造出来、
    //        场景 A 只等"头顶通了"不管脚（于是 A 可能把她留在半埋状态，
    //        B 读到的坐标是歪的）、而且两个场景之间不重置脱困的节流窗口。
    //   两个都修完之后：1000ms 跑 6 次 6/6，200ms 跑 8 次 8/8。
    //
    // 想调回去：MC_REFLEX_MS=1000。
    const reflexMs = Math.max(50, Number(process.env.MC_REFLEX_MS || 200) || 200);
