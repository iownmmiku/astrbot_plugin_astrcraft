...
async function main() {
  ...
  // **矿道不许压到世界最底层**（-64 = 基岩/虚空边界）。
  //
  // 实测踩过（这就是 KNOWN_FLAKY 里「偶发 12/13」的根因之一）：
  // 某些时候 `bp.y` 会算到 -63 → `tunnelY = -64` → 清挖 fill **把基岩挖空**、
  // 矿也铺在 -64 —— 她沿着矿道挖**脚下**那格矿 → 踩空 → **掉出世界**
  // （服务端日志实锤：`AstrBotMine fell out of the world` ×2，两次断连）
  // → 连接断开 → 挖矿任务报「已取消」。
  // 表现是「偶发 12/13，挖矿步红」，**根因在测试场景，不在引擎**。
  //
  // 钳到 -63：矿道在倒数第二层，脚下永远还有基岩兜底（掉下去也只是 1 格）。
  // 正常路径（bp.y - 1 = -61）不受影响。
  const tunnelY = Math.max(bp.y - 1, -63);
