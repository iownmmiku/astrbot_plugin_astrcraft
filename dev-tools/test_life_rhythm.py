...
async def main() -> None:
    ...
    print("\n--- 唤醒：任务结束立刻继续（W7：不再有固定间隔）---")
    # **契约又变了一次，说清楚为什么。**
    #
    # 上一轮我在这里断言"唤醒后不会立刻改主意（承诺间隔 6 秒内不重新决策）"，
    # 那是为了治"乱走乱挖"。但那个药方**治错了病**：
    #   病根 = **重新规划太频繁（每决策一次就改主意）**
    #   不是 = **手上没活**
    # 一刀切压 6 秒的副作用是"她没事做的时候也要干等 6 秒"，
    # 而用户明确要求"**没有任务就立刻发起一个**"。
    #
    # 现在"别频繁改主意"交给**计划连续性**（有计划就执行下一步、不调模型），
    # 时间退避只留给"同一件事反复失败"。所以契约变成：
    #   - **唤醒后立刻继续**（不等任何固定间隔）
    #   - 但不等于"每轮都调模型"——有计划时走的是不花 token 的那条路
    loop._busy_until = time.time() + 300
    loop.wake(reason="测试：任务结束")
    await asyncio.sleep(3)
    early = len(acts) - n0
    check(
        "唤醒后 3 秒内就继续（W7：没有固定间隔，立刻发起）",
        early > 0,
        f"3 秒内新增 {early} 次行动（原来要等满 6 秒）",
    )

    print("\n--- 但'立刻继续'不等于'每轮都调模型' ---")
    # 计划里有步骤时，循环应该**直接执行**，不再问模型。
    # 这是 W7 的核心：用"计划连续性"代替"时间间隔"来避免频繁改主意。
    n1 = len(acts)
    loop._plan = [
        {"skill": "mine_stone", "params": {"count": 3}, "why": "测试用的一步"},
        {"skill": "chop_tree", "params": {"count": 2}, "why": "测试用的第二步"},
    ]
    loop.wake(reason="测试：计划里有步骤")
    await asyncio.sleep(6)
    executed = len(acts) - n1
    check(
        "计划有步骤时直接执行（不调模型、不等待）",
        executed > 0,
        f"6 秒内执行了 {executed} 次（计划剩 {len(loop._plan)} 步）",
    )
    check(
        "执行计划会消耗计划本身（取走即出队）",
        len(loop._plan) < 2,
        f"计划剩 {len(loop._plan)} 步",
    )
    loop._plan = []
