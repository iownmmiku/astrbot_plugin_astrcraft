...
KNOWN_FLAKY = {
    ...
    # **pit 的失败两类**：① [1] 3 格坑垫高的硬场景（概率性攀爬，与用户原报
    # 同源）② 测试按数组位置取任务结果被自主任务插队（n35 实锤：pave 拿到别的
    # 任务的空 result →「说清原因」断言红）—— **② 已修**（runSkill 按 task_id
    # 精确取），修后对照：旧代码 0/2 → 新代码 3/3（n37-39）。失败自带引擎日志。
    "test_pit_escape.js": "偶发：[1] 3 格坑垫高硬场景（概率性攀爬，与用户原报同源）；插队取结果已修（task_id 对照 0/2→3/3），失败自带引擎日志",
