...
KNOWN_FLAKY = {
    ...
    #   · **③ [4] 10 格深井偶发 0 升**（r53，全量 4/5 里的那 1 红）——
    #     **尚未抓到 INFO 现场**（--step4 连跑 5 次未复现，~20% 概率）；
    #     已知线索：WARN 显示她到 [4] 位置后「没有工具」（按理 pick 已 give）
    #     + 「MLG 垫方块自救（落差 7 格）」= 井内有掉落 —— 下轮大样本撞现场
    #   失败自带引擎日志（这轮加的），随时可查。
    "test_pit_escape.js": "偶发约 1/5：[4] 深井 0 升（现场未抓到，线索=到井后「没有工具」+井内掉落；[1]/[2]/[3] 根因已修：forceload 竞态 + task_id 插队），失败自带引擎日志",
