...
KNOWN_FLAKY = {
    ...
    # **pit 的定性（本目标收口时的实测账本）**：
    #   · ① [1] 的根因 = makePit **没 forceload** → fill 在未加载区块**静默无效**
    #     （tp 进去落在没挖的石头里 → 无镐被埋 → spiral 四方向被坑壁静默跳过）
    #     —— 已修（forceload + 挖完 assertBlockAt 验尸）；修前 [1] 失败率 ~40%，
    #     修后 --step1 **5/5**、全量批 [1] 全过
    #   · ② 插队取结果（n35 空 result）→ 已修（task_id 对照 0/2→3/3）
    #   · **③ [4] 深井偶发 0 升**（r53，全量 4/5 里的那 1 红）—— **修复后累计
    #     ~27 次全量里仅此 1 次（≈4%，实测比先前估的 1/5 低）**：
    #     - 隔离 --step4：16 连过（单跑撞不到 → 失败需要全量的累积上下文）
    #     - 全量 INFO 批 ×6：零复现（含 reflex 风暴期）
    #     线索：到井后 WARN「没有工具」（按理 pick 已 give）+「MLG 垫方块自救
    #     （落差 7 格）」井内掉落 —— 现场未抓到，失败自带引擎日志随时可查。
    #   · ④ [3] 零装备坑 `pave → no-finish`（fullpit5 现场）—— 根因 = 反射自救
    #     任务（优先级 0 > 用户 20）排队挤占，120 秒等不到轮次 → **改 300 秒**
    #     （只加耐心，断言不动），修后全量 **5/5**。
    #   失败自带引擎日志（这轮加的），随时可查。
    "test_pit_escape.js": "偶发约 4%（修复后 ~27 次全量仅 r53 一次 [4] 深井 0 升；隔离16连过撞不到，疑需全量累积上下文；线索=到井后「没有工具」+井内掉落）；[1]/[3]/插队根因均已修且 5/5 验证；失败自带引擎日志",
    # **test_mine_return 本轮移出清单**（修后干净批 5/5，不再打偶发标注）：
    # 六个根因全部查实并修复 ——
    #   ① goTo「假到达」（0ms「已在目标附近」）宣布到达却不移动 → 手动走跳补位
    #   ② 停滞计数把掉落当没进展（f7 现场：连升 3 轮仍被判「连续 3 轮没动」）
    #   ③ 跳跃弧线里读高度（g10：引擎读 -50 判出来、测试量到 -51）→ settleFloorY
    #   ④ 一轮四策略全灭当场判死（m32：试了 4 格就放弃，背包里还有 49 圆石）
    #      → 连续 3 轮才如实停
    #   ⑤ 时钟：300 秒不够极限竖井（k29 差 4 格到顶被超时）→ 480 秒（断言零改动）
    #   ⑥ 测试按数组位置取结果被自主任务插队 → 按 task_id 精确取
    # 再红会以真失败现身，届时重新定性。
